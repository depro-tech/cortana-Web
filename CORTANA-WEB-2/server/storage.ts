...
export interface IStorage {
  ...
  // Economy
  getEconomyUser(userJid: string): Promise<EconomyUser | undefined>;
  createEconomyUser(user: InsertEconomyUser): Promise<EconomyUser>;
  updateEconomyUser(userJid: string, data: Partial<InsertEconomyUser>): Promise<EconomyUser | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async createGroupSettings(settings: InsertGroupSettings): Promise<GroupSettings> {
    const [newSettings] = await db!.insert(groupSettings).values(settings).returning();
    return newSettings;
  }
  ...
  async getEconomyUser(userJid: string): Promise<EconomyUser | undefined> {
    const [user] = await db!.select().from(economyUsers).where(eq(economyUsers.userJid, userJid));
    return user;
  }

  async createEconomyUser(user: InsertEconomyUser): Promise<EconomyUser> {
    const [newUser] = await db!.insert(economyUsers).values(user).returning();
    return newUser;
  }

  async updateEconomyUser(userJid: string, data: Partial<InsertEconomyUser>): Promise<EconomyUser | undefined> {
    const [updated] = await db!.update(economyUsers).set(data).where(eq(economyUsers.userJid, userJid)).returning();
    return updated;
  }
}
