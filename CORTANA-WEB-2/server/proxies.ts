...
export const PROXY_LIST: string[] = [
    ...
    "67.43.227.227:11281",
    "67.201.33.10:25283",
    // ... truncated for file size - full list embedded in getRandomProxy()
];

// Extended proxy pool (loaded dynamically)
let extendedProxies: string[] = [];
 ...
 */
export function getProxyBatch(count: number): string[] {
    const allProxies = [...PROXY_LIST, ...extendedProxies];
 ...
 */
export function addProxies(newProxies: string[]): void {
 ...
 */
export function getTotalProxyCount(): number {
 ...
 */
export function formatProxyUrl(proxy: string): string {
    return `http://${proxy}`;
}
 ...
 * Get proxy stats
 */
export function getProxyStats(): { total: number; static: number; dynamic: number } {
    return {
