...
async function handleMessage(sock: ReturnType<typeof makeWASocket>, msg: any, sessionId: string) {
  ...
  try {
    switch (command) {
      ...
      case "menu":
      case "help":
        console.log(`[${sessionId}] Sending menu to ${jid}`);
        try {
          await sendMenu(sock, jid);
          console.log('[${sessionId}] Menu sent successfully');
        } catch (error: any) {
          console.error('[${sessionId}] Error sending menu:', error);
          await sock.sendMessage(jid, { text: "❌ Error sending menu. Please try again." });
        }
        break;
      case "ping":
        const start = Date.now();
        const m = await sock.sendMessage(jid, { text: "🏓 Pinging..." });
        const ping = Date.now() - start;
        if (m?.key) {
          await sock.sendMessage(jid, { text: `🏓 Pong! ${ping}ms`, edit: m.key });
        } else {
