...
* Created : ${s.created_at.split("T")[0]}\n`;
            ...
            case 'brat': {
                ...
                if (!text) return zreply(`— example: ${prefix + command} Arul Tamvan Bjir`);
                zreply(`⏳ Memproses Fitur Brat...`);
                ...
                try {
                    ...
                    exec(`ffmpeg -i ${inputPath} -vf "scale=512:512:force_original_aspect_ratio=decrease" -c:v libwebp -lossless 1 -q:v 80 -preset default -an -vsync 0 ${outputPath}`, async (error) => {
                        if (error) {
                            ...
                            return zreply('Gagal membuat stiker');
                        }
                        await aruloffcx.sendMessage(m.chat, {
            ...
            case 'iqc': {
                ...
                if (!textArgs) {
                    return aruloffcx.sendMessage(m.chat, {
                        text: 'Format salah.\nGunakan: *.iqc jam|batre|pesan*\nContoh: *.iqc 18:00|40|hai hai*'
                    }, { quoted: m });
                }

                const [time, battery, ...messageParts] = textArgs.split('|');

                if (!time || !battery || messageParts.length === 0) {
                    return aruloffcx.sendMessage(m.chat, {
                        text: '❌ Format tidak lengkap!\nContoh: *.iqc 18:00|40|halo*'
                    }, { quoted: m });
                }

                const loading = await aruloffcx.sendMessage(m.chat, { text: '⏳ Generating quoted image...' }, { quoted: m });

                try {
                    const messageText = encodeURIComponent(messageParts.join('|').trim());
                    ...
                    await aruloffcx.sendMessage(
                        m.chat,
                        { image: buffer, caption: '`Succes Bosquu`' },
                        { quoted: m }
                    );

                } catch (err) {
                    console.error('IQC error:', err);
                    await aruloffcx.sendMessage(m.chat, { text: `❌ Terjadi kesalahan: ${err.message}` }, { quoted: m });
                }

                await aruloffcx.sendMessage(m.chat, { delete: loading.key });
                break;
            }

            // CASE NSFW
            case 'nsfw': {
                zreply(`Prosess Mengambil Video NSFW`);

                try {
                    // Fungsi randomNsFw perlu didefinisikan di file terpisah
                    const randomNsFw = async () => {
                        // Implementasi fungsi randomNsFw
                        return [];
                    };

                    const sbe = await randomNsFw();
                    if (!sbe || sbe.length === 0) {
                        return zreply('Tidak dapat mengambil data NSFW');
                    }

                    const cejd = sbe[Math.floor(Math.random() * sbe.length)];
                    await aruloffcx.sendMessage(m.chat, {
...
⭔ Media Url : ${cejd.video_1}`
                    ...
                    zreply('Gagal mengambil konten NSFW');
                }
                break;
            }
            ...
            case 'hentaineko': {
                try {
                    ...
                    await aruloffcx.sendMessage(m.chat, {
                        caption: "sangean lu jir",
                        image: { url: waifudd2.data.url }
                    }, { quoted: m });
                } catch (error) {
                    console.error('Hentaineko error:', error);
                    zreply('Gagal mengambil gambar hentaineko');
                }
                break;
            }
